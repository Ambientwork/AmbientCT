...
if [ ! -f .env ] || [[ "${OVERWRITE:-}" == "true" ]]; then
  ...
  # Allow user to set a custom password interactively
  if [[ "$INTERACTIVE" == "true" ]]; then
    echo "  A secure password has been generated for Orthanc."
    read -r -p "  Use custom password instead? [y/N]: " use_custom
    if [[ "$use_custom" =~ ^[Yy]$ ]]; then
      while true; do
        read -r -s -p "  Enter password (min 16 chars): " custom_pw
        echo ""
        if [ "${#custom_pw}" -lt 16 ]; then
          echo "  ERROR: Password must be at least 16 characters."
          continue
        fi
        case "$custom_pw" in
          changeme-on-first-run|changeme|password|admin|orthanc|123456*)
            echo "  ERROR: This is a known weak password. Choose a stronger one."
            continue ;;
        esac
        read -r -s -p "  Confirm password: " custom_pw2
        echo ""
        if [ "$custom_pw" != "$custom_pw2" ]; then
          echo "  ERROR: Passwords do not match."
          continue
        fi
        ORTHANC_PASSWORD="$custom_pw"
        break
      done
    fi
  fi
...
# Validate the .env file (catches default passwords, missing vars, port conflicts)
